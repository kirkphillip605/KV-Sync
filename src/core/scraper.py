...
class SongScraper:
            ...
            try:
                ...
                for song_row in purchased_songs:
                    try:
                        ...
                        logger.warning(f"Failed to extract song data from row: {e}")
                        continue
                ...
                if attempt == max_retries - 1:
                    logger.error(f"All attempts failed for page {page_number}: {e}")
                    return [], False
    ...
    def scrape_all_pages(self, last_song_id=None, validate=False, progress_callback=None):
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    if progress_callback:
                        ...
                        progress_callback(progress, f"Scraped page {completed_pages}/{total_pages}")
                    
                    if found_last_song:
                        logger.info(f"Found last song ID {last_song_id}, stopping scrape")
                        break
                    ...
                    if progress_callback:
                        progress = int((completed_pages / total_pages) * 100)
                        progress_callback(progress, f"Failed page {page_number}, continuing...")
